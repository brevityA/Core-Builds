...
const STEPS = 6;
const CONFIGURATOR_VERSION = '2.86';
...
// Apex Mixed PSE stack — ported verbatim from Templates/Torbox/Nightly/Single/core-nexus-4k-apex-mixed.json.
// configurator/tests/apex-mixed.test.mjs asserts this stays byte-identical to the template (golden link).
const APEX_MIXED_PSES = [
  {"expression": "/* 4K */ resolution(streams, '2160p')", "enabled": true},
  {"expression": "/* 1080p */ resolution(streams, '1080p')", "enabled": true},
  {"expression": "/* 720p */ resolution(streams, '720p')", "enabled": true},
  {"expression": "/* Remux first */ quality(streams, 'Bluray REMUX', 'BluRay')", "enabled": true},
  {"expression": "/* Web-DL over WebRip */ quality(streams, 'WEB-DL', 'WEBRip')", "enabled": true},
  {"expression": "/* HDR / DV */ visualTag(streams, 'HDR+DV', 'DV', 'HDR10+', 'HDR10', 'HDR')", "enabled": true},
  {"expression": "/* HEVC / AV1 preferred */ encode(streams, 'HEVC', 'AV1')", "enabled": true},
  {"expression": "/* Cached priority */ cached(streams)", "enabled": true},
  ...
  {"expression": "/* 4K WEB-DL HDR — adaptive bitrate + age decay (pow 0.95/d) */ count(resolution(visualTag(quality(streams,'WEB-DL'),'HDR+DV','DV','HDR10+','HDR10','HDR'),'2160p'))>=4 ? bitrate(resolution(visualTag(quality(streams,'WEB-DL'),'HDR+DV','DV','HDR10+','HDR10','HDR'),'2160p'), q1(values(resolution(visualTag(quality(streams,'WEB-DL'),'HDR+DV','DV','HDR10+','HDR10','HDR'),'2160p'),'bitrate')) - 1.5*iqr(values(resolution(visualTag(quality(streams,'WEB-DL'),'HDR+DV','DV','HDR10+','HDR10','HDR'),'2160p'),'bitrate')), q3(values(resolution(visualTag(quality(streams,'WEB-DL'),'HDR+DV','DV','HDR10+','HDR10','HDR'),'2160p'),'bitrate')) + 1.5*iqr(values(resolution(visualTag(quality(streams,'WEB-DL'),'HDR+DV','DV','HDR10+','HDR10','HDR'),'2160p'),'bitrate'))) : count(resolution(visualTag(quality(streams,'WEB-DL'),'HDR+DV','DV','HDR10+','HDR10','HDR'),'2160p'))>0 ? bitrate(resolution(visualTag(quality(streams,'WEB-DL'),'HDR+DV','DV','HDR10+','HDR10','HDR'),'2160p'), min(values(resolution(visualTag(quality(streams,'WEB-DL'),'HDR+DV','DV','HDR10+','HDR10','HDR'),'2160p'),'bitrate'))*0.80, max(values(resolution(visualTag(quality(streams,'WEB-DL'),'HDR+DV','DV','HDR10+','HDR10','HDR'),'2160p'),'bitrate'))*1.20) : ((count(bitrate(resolution(visualTag(quality(streams,'WEB-DL'),'HDR+DV','DV','HDR10+','HDR10','HDR'),'2160p'), median(values(bitrate(resolution(visualTag(quality(streams,'WEB-DL'),'HDR+DV','DV','HDR10+','HDR10','HDR'),'2160p'),'5Mbps'),'bitrate'))*(1-0.4*pow(0.95,daysSinceRelease)), median(values(bitrate(resolution(visualTag(quality(streams,'WEB-DL'),'HDR+DV','DV','HDR10+','HDR10','HDR'),'2160p'),'5Mbps'),'bitrate'))*(1+0.4*pow(0.95,daysSinceRelease))))>=1 ? bitrate(resolution(visualTag(quality(streams,'WEB-DL'),'HDR+DV','DV','HDR10+','HDR10','HDR'),'2160p'), median(values(bitrate(resolution(visualTag(quality(streams,'WEB-DL'),'HDR+DV','DV','HDR10+','HDR10','HDR'),'2160p'),'5Mbps'),'bitrate'))*(1-0.4*pow(0.95,daysSinceRelease)) : []", "enabled": true},
  ...
  {"expression": "/* Limit 4K Bluray */ slice(resolution(quality(streams, 'Bluray'), '2160p'), 0, 3)", "enabled": true},
  {"expression": "/* Limit 4K WEB-DL */ slice(resolution(quality(streams, 'WEB-DL'), '2160p'), 0, 3)", "enabled": true},
  {"expression": "/* Limit 1080p Remux */ slice(resolution(quality(streams, 'Bluray REMUX'), '1080p'), 0, 3)", "enabled": true},
  {"expression": "/* Limit 1080p Bluray */ slice(resolution(quality(streams, 'Bluray'), '1080p'), 0, 3)", "enabled": true},
  {"expression": "/* Limit 1080p WEB-DL */ slice(resolution(quality(streams, 'WEB-DL'), '1080p'), 0, 3)", "enabled": true},
  {"expression": "/* Limit 720p WEB-DL */ slice(resolution(quality(streams, 'WEB-DL'), '720p'), 0, 3)", "enabled": true},
  {"expression": "/* Limit 720p WEBRip */ slice(resolution(quality(streams, 'WEBRip'), '720p'), 0, 3)", "enabled": true},
  {"expression": "/*Codec Efficiency Booster*/ merge(encode(resolution(quality(negate(merge(library(streams),seadex(streams)),cached(streams)),'Bluray REMUX'),'1080p'),'HEVC','AV1'),encode(resolution(quality(negate(merge(library(streams),seadex(streams)),cached(streams)),'WEB-DL','WEBRip'),'1080p'),'HEVC','AV1'),encode(resolution(quality(negate(merge(library(streams),seadex(streams)),cached(streams)),'Bluray REMUX'),'720p'),'HEVC','AV1'),encode(resolution(quality(negate(merge(library(streams),seadex(streams)),cached(streams)),'WEB-DL','WEBRip'),'720p'),'HEVC','AV1'))", "enabled": true},
  {"expression": "/*IMAX pin*/ count(visualTag(streams, 'IMAX')) > 0 ? pin(visualTag(streams, 'IMAX'), 'top') : []", "enabled": true},
  {"expression": "/* Extra Cached HQ */ negate(perGroup(negate(merge(library(streams), uncached(streams)), quality(streams, 'Bluray REMUX', 'Bluray', 'WEB-DL', 'WEBRip')), 'resolution', 5), negate(merge(library(streams), uncached(streams)), quality(streams, 'Bluray REMUX', 'Bluray', 'WEB-DL', 'WEBRip')))", "enabled": true},
  {"expression": "/* Audio channel priority */ audioTag(streams, 'Atmos', 'DTS:X', 'TrueHD', 'DTS-HD MA', 'FLAC')", "enabled": true},
  {"enabled": true, "expression": "/* 576p */ resolution(streams, '576p')"},
  {"expression": "/* 480p */ resolution(streams, '480p')", "enabled": true},
  {"expression": "/* 240p */ resolution(streams, '240p')", "enabled": true},
  {"enabled": true, "expression": "/* 576p Quality */ quality(streams, 'WEB-DL', 'WEBRip', 'HDTV', 'HDRip', 'DVDRip', 'HC HD-Rip', 'CAM', 'TS', 'TC', 'SCR', 'Unknown')"},
  {"expression": "/* 480p Quality */ quality(streams, 'WEB-DL', 'WEBRip', 'HDTV', 'HDRip', 'DVDRip', 'HC HD-Rip', 'CAM', 'TS', 'TC', 'SCR', 'Unknown')", "enabled": true},
  {"expression": "/* 240p Quality */ quality(streams, 'HDTV', 'HDRip', 'DVDRip', 'HC HD-Rip', 'CAM', 'TS', 'TC', 'SCR', 'Unknown')", "enabled": true},
  {"enabled": true, "expression": "/* Limit 576p results */ slice(resolution(streams, '576p'), 0, 3)"},
  {"expression": "/* Limit 480p results */ slice(resolution(streams, '480p'), 0, 3)", "enabled": true},
  {"expression": "/* Limit 240p results */ slice(resolution(streams, '240p'), 0, 3)", "enabled": true}
];
...
function eses() {
  ...
  out.push(
    ...
    { enabled:true, expression:"/* CB | Kill Ambiguous Packs When Non-Pack Exist */ count(negate(streams, seasonPack(streams))) > 0 ? negate(seasonPack(streams), episodePack(streams)) : []" },
...
function pses() {
  ...
  // Apex Mixed architecture — flagship IQR × adaptive stack (ported from the 4K Apex Mixed nightly, kept in sync by tests/apex-mixed.test.mjs).
  if (S.pseArch === 'apex-mixed') {
    out.push(...APEX_MIXED_PSES.map(p => ({ ...p })));
    return out;
  }

  if (res === '4k' && S.pseArch === 'iqr') {
    const iqr2 = (label, pool, floor, decay, decayFloor) => {
      const b = `q1(values(${pool},'bitrate'))-1.5*iqr(values(${pool},'bitrate'))`, t = `q3(values(${pool},'bitrate'))+1.5*iqr(values(${pool},'bitrate'))`;
      const iqrBranch = floor ? `size(bitrate(${pool},${b},${t}),'${floor}')` : `bitrate(${pool},${b},${t})`;
      const minmax = floor ? `size(bitrate(${pool},min(values(${pool},'bitrate'))*0.80,max(values(${pool},'bitrate'))*1.20),'${floor}')` : `bitrate(${pool},min(values(${pool},'bitrate'))*0.80,max(values(${pool},'bitrate'))*1.20)`;
      let expr = `/*${label}*/ count(${pool})>=4?${iqrBranch}:count(${pool})>0?${minmax}:`;
      if (decay) {
        const mFloor = decayFloor || '5Mbps';
        expr += `(count(bitrate(${pool},median(values(bitrate(${pool},'${mFloor}'),'bitrate'))*(1-0.4*pow(0.95,daysSinceRelease)),median(values(bitrate(${pool},'${mFloor}'),'bitrate'))*(1+0.4*pow(0.95,daysSinceRelease))))>=1?bitrate(${pool},median(values(bitrate(${pool},'${mFloor}'),'bitrate'))*(1-0.4*pow(0.95,daysSinceRelease))):[])`;
      } else { expr += '[]'; }
    ...
    if (dv) out.push(iqr2('S-Tier 4K REMUX DV — IQR Tukey fence', "visualTag(quality(resolution(streams,'2160p'),'BluRay REMUX'),'DV','HDR+DV')", '15GB', false));
    out.push(
      iqr2('S-Tier 4K REMUX — IQR Tukey fence', "resolution(quality(streams,'BluRay REMUX'),'2160p')", '15GB', false),
      iqr2('A-Tier 4K WEB-DL HDR — IQR + pow() decay', "resolution(visualTag(quality(streams,'WEB-DL'),'HDR+DV','DV','HDR10+','HDR10','HDR'),'2160p')", null, true, '5Mbps'),
      iqr2('B-Tier 4K WEB-DL SDR — IQR + pow() decay', "resolution(quality(streams,'WEB-DL'),'2160p')", null, true, '5Mbps'),
      iqr2('C-Tier 4K WEBRip HDR — IQR + pow() decay', "resolution(visualTag(quality(streams,'WEBRip'),'HDR+DV','DV','HDR10+','HDR10','HDR'),'2160p')", null, true, '5Mbps'),
      iqr2('D-Tier 4K WEBRip SDR — IQR', "resolution(quality(streams,'WEBRip'),'2160p')", null, false),
      { enabled:true, expression:"/* E-Tier Any 4K */ resolution(streams,'2160p')" },
      iqr2('S-Tier 1080p REMUX — IQR Tukey fence', "resolution(quality(streams,'BluRay REMUX'),'1080p')", '8GB', false),
      iqr2('A-Tier 1080p WEB-DL — IQR + pow() decay', "resolution(quality(streams,'WEB-DL'),'1080p')", null, true, '1Mbps'),
    ...
    const iqr1 = (label, pool, floor, decay, decayFloor) => {
      const b = `q1(values(${pool},'bitrate'))-1.5*iqr(values(${pool},'bitrate'))`, t = `q3(values(${pool},'bitrate'))+1.5*iqr(values(${pool},'bitrate'))`;
      const iqrBranch = floor ? `size(bitrate(${pool},${b},${t}),'${floor}')` : `bitrate(${pool},${b},${t})`;
      const minmax = floor ? `size(bitrate(${pool},min(values(${pool},'bitrate'))*0.80,max(values(${pool},'bitrate'))*1.20),'${floor}')` : `bitrate(${pool},min(values(${pool},'bitrate'))*0.80,max(values(${pool},'bitrate'))*1.20)`;
      let expr = `/*${label}*/ count(${pool})>=4?${iqrBranch}:count(${pool})>0?${minmax}:`;
      if (decay) {
        const mFloor = decayFloor || '1Mbps';
        expr += `(count(bitrate(${pool},median(values(bitrate(${pool},'${mFloor}'),'bitrate'))*(1-0.4*pow(0.95,daysSinceRelease)),median(values(bitrate(${pool},'${mFloor}'),'bitrate'))*(1+0.4*pow(0.95,daysSinceRelease))))>=1?bitrate(${pool},median(values(bitrate(${pool},'${mFloor}'),'bitrate'))*(1-0.4*pow(0.95,daysSinceRelease))):[])`;
      } else { expr += '[]'; }
      return { enabled:true, expression:expr };
    };
    out.push(pin1080Elite, pinLQ,
      iqr1('S-Tier 1080p REMUX — IQR Tukey fence', "resolution(quality(streams,'BluRay REMUX'),'1080p')", '8GB', false),
      iqr1('A-Tier 1080p WEB-DL — IQR + pow() decay', "resolution(quality(streams,'WEB-DL'),'1080p')", null, true, '1Mbps'),
      { enabled:true, expression:"/* B-Tier 1080p WEBRip or BluRay */ quality(resolution(streams,'1080p'),'WEBRip','BluRay')" },
      { enabled:true, expression:"/* C-Tier Any 1080p */ resolution(streams,'1080p')" },
...
function build() {
  ...
  const standaloneOnly = useBase ? {} : {
    ...
    sortCriteria: (function(){ const d='desc',qf=S.qualityFirst,rf=S.resolutionFirst, rq=(qf||((S.resolution==='mixed'||S.pseArch==='apex-mixed')&&!rf))?[{key:'quality',direction:d},{key:'resolution',direction:d}]:[{key:'resolution',direction:d},{key:'quality',direction:d}], rfPre=rf?rq:[],rfPost=rf?[]:rq, isFree=S.service==='p2p'||S.service==='http';
      ...
      const lb = S.libraryBoost || 'default';
...
function showCompareTemplates() {
  ...
  function updateComparison() {
    ...
    const rows = [
      ...
      { l: 'pow() Decay', a: yn(a.pw),      b: yn(b.pw),     raw: true },
