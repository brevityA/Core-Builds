...
function deriveService() {
  const PRIMARY = ['torbox-pro','torbox-ess','alldebrid','realdebrid','premiumize','debridlink','easynews','offcloud','debridio','debrider','easydebrid','pikpak','seedr','usenet'];
...
function splashHtml() {
  ...
  return `<div class="card splash hybrid-splash">
    ...
    <div class="splash-doors splash-anim splash-anim-d6">
      ...
      <a class="splash-door core-tool-door" href="../tools/banner-studio/"><div class="splash-door-icon"><span style="font-size:20px;line-height:1" aria-hidden="true">🎬</span></div><div class="splash-door-text"><div class="splash-door-title">Banner Studio <span class="splash-door-tag" style="background:rgba(248,113,113,.12);color:#fca5a5;border:1px solid rgba(248,113,113,.3)">17 brands</span></div><div class="splash-door-desc">Netflix-style streaming banners for any major streamer — render &amp; export locally.</div></div></a>
...
function getDebridInputs() {
  ...
  S.optionalScrapers.forEach(sid => {
    ...
    if (d && d.presetType === 'nzbhydra' && !ids.includes('nzbhydraApiKey')) ids.push('nzbhydraApiKey');
  });
...
function presets() {
  ...
  const list = [
    ...
    ...S.optionalScrapers.filter(sid => OPTIONAL_SCRAPER_DEFS.find(x => x.id === sid && x.credKey && !x.apiUrl && x.presetType !== 'nzbhydra')).map(sid => {
      ...
      // Debridio catalogs/watchtower: `debridioApiKey` is required upstream with no default,
      // so the jackett/prowlarr rule applies — no key, no preset. Unlike those two, S.creds.debridio
      // DOES have a UI writer, so this branch is reachable. These three were introduced as keyless
      // toggles emitting {name,timeout} only; the host rejected the whole config on save.
      if (d.id === 'debridio-tmdb' || d.id === 'debridio-tvdb' || d.id === 'debridio-watchtower') {
        if (!S.creds.debridio) return null;
        const meta = d.id === 'debridio-watchtower'
          ? { resources:['stream'] }
          : { resources:['catalog','meta'], category:'meta_catalogs' };
        return { type:d.presetType, instanceId:`${d.id}-opt`, enabled:true, options:{ name:d.label, timeout:5000, debridioApiKey:S.creds.debridio }, ...meta };
      }
      return null;
    }).filter(Boolean),
    ...S.optionalScrapers.filter(sid => OPTIONAL_SCRAPER_DEFS.find(x => x.id === sid && x.presetType === 'nzbhydra')).map(sid => {
...
function services() {
  ...
  return [
    ...
    {id:'easynews', enabled: svc==='easynews' || svc==='usenet' || (isMulti && m.includes('easynews')), credentials: (svc==='easynews' || svc==='usenet' || (isMulti && m.includes('easynews'))) && S.creds.easynews ? { username:S.creds.easynews, password:S.creds.easynewsPass||'' } : {}},
...
function showPreflightModal(findings, { actionLabel = 'Continue anyway' } = {}) {
  ...
  const rows = findings.map(f => {
    ...
    return `<div style="padding:11px 13px;border-radius:10px;background:${s.bg};border:1px solid ${s.border};margin-bottom:8px">
      <div style="display:flex;align-items:baseline;gap:8px;flex-wrap:wrap;margin-bottom:${f.detail || f.fix ? '4px' : '0'}">
        <span style="font-size:.7rem;font-weight:900;letter-spacing:.1em;text-transform:uppercase;color:${s.color};flex-shrink:0">${s.label}</span>
  ...
  return new Promise(resolve => {
    ...
    overlay.innerHTML = `<div class="fastlane-panel" role="dialog" aria-modal="true" aria-labelledby="preflightTitle" style="max-width:600px">
      ...
      ${blocked ? '<div style="font-size:.7rem;color:var(--th-red,#f87171);text-align:center;margin-top:9px">A blocking issue means this cannot produce a working setup.</div>' : ''}
    </div>`;
