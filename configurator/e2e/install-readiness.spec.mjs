...
test('review Deploy stays disabled with a precise message while the debrid key is missing', async ({ page }) => {
  ...
  await page.locator('#btnNext').click();
  await page.locator('[data-action="skip-content"]').click();
  await page.locator('#btnNext').click(); // -> apis
  await page.locator('#btnNext').click(); // -> review
  ...
  await page.locator('[data-action="jump-step"][data-step="5"]').click();
...
test('Guided Setup un-disables Deploy live when the key is typed, then posts', async ({ page }) => {
  ...
  await page.locator('#btnNext').click();
  await page.locator('.device-card[data-val="generic"]').click();
  await page.locator('#btnNext').click(); // -> apis (simple-mode step 3)
...
test('the free P2P lane is exempt from the key gate', async ({ page }) => {
  ...
  await fresh(page);
  await page.locator('[data-action="custom-start"]').click();
  await page.locator('label[for="o_p2p"]').click();
  // ElfHosted does not serve P2P — an explicit compatible host first, exactly
  // as the UI would guide a real user.
  await page.locator('#btnNext').click();
  await page.locator('.device-card[data-val="generic"]').click();
  await page.locator('#btnNext').click();
  await page.locator('label[for="o_1080p"]').click();
