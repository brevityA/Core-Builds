...
function requiredBeyond(presetType) {
  return AIO_PRESET_REQUIRED_OPTIONS[presetType] || [];
}
...
test('safe add-on candidates from audit have no extra required options', () => {
  const candidates = [
    ...
    'opensubtitles', 'tmdb-collections', 'torbox', 'torbox-search',
    'usa-tv', 'usa-tv-next',
  ];
  const unsafe = candidates.filter(id => (AIO_PRESET_REQUIRED_OPTIONS[id] || []).length > 0);
