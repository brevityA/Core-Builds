...
function buildPresets(input) {
  ...
  const extrasOn = sid => optionalScrapers.includes(sid);
  const animeContent = content === 'anime' || content === 'all' || content === 'mixed';
  ...
  if (isUsenet) {
    const usenetList = [
      ...
      ...optionalScrapers.filter(sid => OPTIONAL_SCRAPER_DEFS.find(x => x.id === sid && ['easynews','easynewsPlus'].includes(x.id))).map(sid => {
        const d = OPTIONAL_SCRAPER_DEFS.find(x => x.id === sid);
