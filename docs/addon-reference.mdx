...
| `torbox-search` | Legacy built-in preset, v2.31.1 lane only | `[stream]` | No | Removed in v2.32; the separate Newznab option labelled TorBox Search needs its own endpoint/key validation |
...
`comet`, `zilean`, `stremthruTorz`, `torrent-galaxy`, `eztv`, `hdhub`, `knaben`, `torrents-db`, `seadex`, `animeTosho`, `nekobt`, `newznab` — these all default to stream-only. Setting `resources: ['stream']` explicitly is harmless but not required.
...
*Last updated: v2.9.5 · Sources: [AIOStreams presets](https://github.com/Viren070/AIOStreams/tree/main/packages/core/src/presets)*
