...
`comet`, `zilean`, `stremthruTorz`, `torrent-galaxy`, `eztv`, `hdhub`, `knaben`, `seadex`, `animeTosho`, `nekobt`, `torbox-search`, `newznab` — these all default to stream-only. Setting `resources: ['stream']` explicitly is harmless but not required.
...
*Last updated: v2.9.4 · Sources: [AIOStreams presets](https://github.com/Viren070/AIOStreams/tree/main/packages/core/src/presets)*
