...
Use the complete configuration interface for device profiles, filters, expressions, sorting, formatters, subtitles, catalogs, proxy behaviour, language handling, and other advanced controls.
...
With neither value, title and year matching are disabled and metadata matching may be less accurate. The preflight warning can be accepted if that trade-off is intentional.
