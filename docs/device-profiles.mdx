...
description: "Template recommendations and codec constraints for Samsung TV, Apple TV, Fire Stick, Google TV Streamer, Android TV, and Chromecast."
---
...
<Info>
  **Stable template:** [Samsung RU7100 4K](/template-directory#torbox-pro) — full APEX IQR Tukey fence PSE stack, FLAC/AAC native audio, tuned specifically for the RU7100 (2019) and compatible Tizen models. Promoted from Nightly to stable in v2.9.x.
</Info>
...
**Recommended template:** [Google TV Streamer 4K](/template-directory#torbox-pro)

| Feature | Support |
|---|---|
| Dolby Vision | Yes — natively supported |
| AV1 | Yes — hardware decode |
| HDR10+ | Yes |
| HDR10 / HLG | Yes |
| HEVC / AVC | Hardware decode |
| TrueHD / DTS:X / DTS-HD MA / FLAC | Hard-excluded — no lossless audio passthrough |
| DD+ Atmos | Preferred — native decode, audio ceiling |
| Audio channels | 5.1 max |

The Google TV Streamer 4K template is based on 4K Apex with full IQR Tukey fence PSE stack, but excludes lossless audio formats and caps channels at 5.1. The Audio Pinnacle PSE is removed since the device cannot benefit from lossless audio ranking.

<Info>
  The Google TV Streamer supports DV natively — no DV-Only Kill ESE is needed. DV streams are prioritised in the visual tag sort order.
</Info>

---

## Android TV / Google TV (Other)

For other Android TV devices (Shield, Chromecast with Google TV, Sony/Philips/TCL running Google TV) that don't have a dedicated template:
...
<AccordionGroup>
  <Accordion title="DV-Only Kill ESE">
    Enabled by default on Samsung templates. Removes streams where Dolby Vision is the only signal layer (no HDR10 fallback). Safe to disable if your device supports DV natively (Fire Stick 4K Max, Google TV Streamer, Chromecast with Google TV, Apple TV).
  </Accordion>
