...
<Steps>
  ...
  <Step title="ESEs remove junk">
    **Excluded Stream Expressions** run first. Streams matching any ESE are removed entirely before ranking starts. Core Builds standard ESEs kill: CAM recordings, YouTube rips, 3D streams, sub-720p resolutions, season packs on episode queries, bad NZBs, and more.

    Standard: 24 ESEs. Lite: 12 (hard kills only — no quality gates).
  </Step>
...
Lite templates remove the quality-gate ESEs (low bitrate, low seeders, upscaled 4K, bad BluRay encodes). Only hard kills remain (CAM, YouTube, 3D, sub-720p). More results come through — but without quality filtering, some bad encodes will appear.
