...
<Accordion title="Stadium sync">
  A venue bridge in this repo, `packages/stadium-sync`, turns a Daktronics console, a Sportzcast ScoreLink, or a tablet pad into a Core Line JSON feed and a full-screen gym bug. The shipping 1.3.0 APK still blocks raw LAN URLs — open `/display` on the gym TV, or pair a hostname. The market read and the allowlist the app still needs are on [Core Line](/core-line).
</Accordion>
