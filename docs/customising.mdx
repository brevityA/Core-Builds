...
Result caps live in `resultLimits` — `global` caps the whole list, `resolution` caps each resolution bucket, and `mode` decides how the two combine. Increase for a wider list or decrease for a tighter one:

```json
"resultLimits": { "global": 40, "resolution": 15, "mode": "conjunctive" }
```

Do not use the old `maxResults` / `maxResultsPerResolution` spellings: they were never AIOStreams schema keys, so hosts silently strip them and they have no effect.
