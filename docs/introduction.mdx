  ...
  <div style={{ display: 'flex', justifyContent: 'center', marginTop: '24px', marginBottom: '12px' }}>
    <div style={{ display: 'flex', background: '#0f172a', borderRadius: '10px', border: '1px solid #1e3a5f', overflow: 'hidden', width: '100%', maxWidth: '480px' }}>
      ...
      <div style={{ flex: 1, textAlign: 'center', padding: '18px 0' }}>
        <div style={{ fontSize: '20px', fontWeight: '800', color: '#60a5fa' }}>v2.8.8</div>
...
<Info>
  **What's new in v2.8.8:** Regex configuration corrected across all templates — full `{name, pattern, score}` entries restored in `rankedRegexPatterns`, `preferredRegexPatterns` reinstated, 12 `excludedRegexPatterns` restored (including lookbehind), and `syncedExcludedRegexUrls` to Tamtaro re-added. elfhosted's whitelist covers all these patterns — the prior removals in v2.8.6/v2.8.7 were unnecessary and degraded quality ranking.
</Info>
