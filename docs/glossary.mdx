...
<AccordionGroup>
  ...
  <Accordion title="SeaDex">
    An anime-only best-release database. Tracks which fansub/encode group produced the definitive release for each anime title. Used in Anime templates via `enableSeadex`. (The old `seadexBestOnly` key was never part of the AIOStreams schema — hosts strip it — so it never restricted anything; retired from all templates in v3.8.)
  </Accordion>
