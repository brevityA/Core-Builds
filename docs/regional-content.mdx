...
AIOStreams and stream addons (Comet, Zilean) are **stream-only** — they have no catalog or Discover output. You need a separate catalog addon to populate regional Discover rows.
