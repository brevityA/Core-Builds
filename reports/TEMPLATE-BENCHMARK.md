...
| `variant-apex-failover-on` | `failover.maxAttempts: unset (=3) → 5` | `failover_max_attempts None → 5` | adopted 2026-09-22: Apex now ships conservative failover, so the variant escalates the attempt budget instead of enabling the feature |
| `variant-apex-failover-parallel` | `failover.parallel: unset (=1) → 2` | exactly that field | sister variant: 2-in-flight vs sequential on the failing path |
| `variant-apex-failover-crosstype` | `failover.allowCrossType: unset (=false) → true` | exactly that field | sister variant: dead debrid picks may fail over into Usenet and vice versa |
| `variant-apex-failover-early` | `failover.position: unset (=last) → beforeLimiting` | exactly that field | sister variant: targets drawn from the pre-limit pool vs final list only |
...
existed in upstream's v2.34.0 schema but that **no Core Builds template set**.
(Apex adopted conservative `failover` on 2026-09-22, so the failover row and its
three sisters now escalate from that baseline instead.)
