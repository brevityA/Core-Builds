...
- **All 35 active templates — synced expression URLs removed** — `syncedPreferredStreamExpressionUrls`, `syncedRankedStreamExpressionUrls`, `syncedIncludedStreamExpressionUrls`, and `syncedExcludedStreamExpressionUrls` all pointed to Tamtaro/Vidhin05 GitHub URLs. Tamtaro PSEs use `rseMatched(streams, 'tier_name')` to match RSE tier names defined in Vidhin05's expressions.json. Public AIOStreams instances (elfhosted, fortheweak.cloud) block these raw GitHub URLs — RSE tier definitions never load, so every `rseMatched` call with a tier name throws "Invalid stream expression" at startup. All four synced fields removed. Templates now rely entirely on their own inline expressions, which are comprehensive and were already the primary logic.
