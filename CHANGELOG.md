...
- **The three advanced/labs goldens carried stale pattern entries** — the CI run on the fix head showed `torbox-4k-iqr`, `torbox-4k-apex-mixed` and `torbox-mixed-apex-mixed` failing with 15 extra lines each (`Radarr HD Bluray T1`/`T2`, `TrueHD Exclude Groups`) that the current app pipeline no longer emits: the local rebaseline was captured against an intermediate bundle, not the final re-pinned data. Refreshed via a headless jsdom run of the built bundle (one fresh app instance per combo, matching the spec's per-page isolation): the other 12 goldens reproduce byte-identically, proving the harness is CI-equivalent; zero hand-editing.
