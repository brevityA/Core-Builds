...
- **Configurator/CLI embedded regex copies now ride the same pipeline** — the v2.34.0 snapshot re-pin refreshed the standalone templates, allowlist and reviewed list, but `configurator/src/js/app.js`'s `RANKED_REGEX_*`/`EXCLUDED_REGEX` consts and `cli/data/ranked-regex.json` were not sync targets: their text for `BR-DISK`, `Radarr HD Bluray T1/T2` and `TrueHD Exclude Groups` stayed pre-refresh while the host allowlist moved, so the host gate (correctly) stripped those patterns from generated advanced/labs output and every golden/CLI-equivalence check downstream shifted. `sync_ranked_regex.py` now treats both as first-class derived copies (refreshed in place, membership/order never touched) and its `--check` mode covers them; `sync_template_collection.py --regex-allowlist` regenerates both allowlist mirrors (configurator + `packages/core`) together, closing the divergence that let the two copies drift. Goldens re-derived from the fixed pipeline: the three advanced/labs fixtures now carry the refreshed `TrueHD Exclude Groups` text, 12 stay byte-identical; CLI suite 74/0, packages/core 35/0, configurator 484/0 against the same fixtures.
...
- **C3 flake root-caused and un-quarantined** (#738) — every recorded failure was a hydration race, never a security assertion. The fixed 900 ms/600 ms sleeps are replaced by explicit gates (the password prompt is waited for conditionally — clicked when it renders, with the immediate-build path accepted when the flow already progressed), the dialog handler attaches before navigation instead of after it, and BOTH modal-driving clicks (`[data-action="open-express-lane"]` and `#expressGo`) are wrapped in `toPass` retry gates so a painted-but-unbound or re-render-swallowed button is re-clicked until observable progress — the mobile project swallowed `#expressGo` clicks at sample #3. The `userPosts` proof-of-route and all inertness/legibility assertions are untouched; nothing was loosened to get green. DoD: 5 consecutive green CI runs on the PR signal.
- **The three advanced/labs goldens carried stale pattern entries** — the CI run on the fix head showed `torbox-4k-iqr`, `torbox-4k-apex-mixed` and `torbox-mixed-apex-mixed` failing with 15 extra lines each (`Radarr HD Bluray T1`/`T2`, `TrueHD Exclude Groups`) that the current app pipeline no longer emits: the local rebaseline was captured against an intermediate bundle, not the final re-pinned data. Refreshed via a headless jsdom run of the built bundle (one fresh app instance per combo, matching the spec's per-page isolation): the other 12 goldens reproduce byte-identically, proving the harness is CI-equivalent; zero hand-editing. *(Superseded the same day — the entries are restored, with refreshed text, by the configurator-copy fix below; stripping them was treating the symptom.)*
