...
- **C3 flake root-caused and un-quarantined** (#738) — every recorded failure was a hydration race, never a security assertion. The fixed 900 ms/600 ms sleeps are replaced by explicit gates (the password prompt is now waited for conditionally — clicking `.pwd-go` if it renders, accepting the immediate-build path if the origin already holds a token, as observed once on the mobile project — and a content-poll for the rendered reason), the dialog handler attaches before navigation instead of after it, and the Express-lane open click is wrapped in `toPass` so a painted-but-unbound button is retried until `#expressLaneModal` actually renders. The `userPosts` proof-of-route and all inertness/legibility assertions are untouched; nothing was loosened to get green. DoD: 5 consecutive green CI runs on the PR signal.
